/* DSH compatibility bundle: IMA-BASE-1 */
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		var UnsupportedHarnessClientError = class extends Error {
			missing;
			code = "IMA_UNSUPPORTED_HARNESS_CLIENT";
			constructor(missing) {
		...
		function unwrapGateway(result) {
			if (result.ok) return result.value;
			throw new Error(messageOf(result.error));
		}
		function createLegacyCredentialsAdapter(credentials) {
			return {
		...
		function createGatewayCredentialsAdapter(credentials) {
			return {
		...
		function createClientCredentials(ctx) {
			const remoteRoot = safeGet(ctx, "remote") ?? ctx.remote;
			const remoteCredentials = safeGet(ctx, "remote.credentials") ?? (isRecord(remoteRoot) ? remoteRoot.credentials : void 0);
			if (hasMethods(remoteCredentials, ["describe", "set"])) return createGatewayCredentialsAdapter(remoteCredentials);
			const connection = safeGet(ctx, "connection");
			const legacyCredentials = isRecord(connection) && isRecord(connection.api) ? connection.api.credentials : void 0;
			if (hasMethods(legacyCredentials, ["describe", "set"])) return createLegacyCredentialsAdapter(legacyCredentials);
			throw new UnsupportedHarnessClientError(["remote.credentials.describe/set", "connection.api.credentials.describe/set"]);
		}
		function getSlotsClient(ctx) {
			const slots = safeGet(ctx, "slots") ?? ctx.slots;
			if (!hasMethods(slots, ["inject", "register"])) throw new UnsupportedHarnessClientError(["slots.inject/register"]);
			return slots;
		}
		//#endregion
		...
		*/
		...
		exports.UnsupportedHarnessClientError = UnsupportedHarnessClientError;
		exports.apply = apply;
		exports.createClientCredentials = createClientCredentials;
		exports.createGatewayCredentialsAdapter = createGatewayCredentialsAdapter;
		exports.createLegacyCredentialsAdapter = createLegacyCredentialsAdapter;
