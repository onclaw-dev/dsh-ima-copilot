...
//#endregion
//#region src/credential-api.ts
const IMA_CREDENTIAL_API_PREFIX = "/api/ima-copilot/credentials";
const MAX_BODY_BYTES = 131072;
const ALLOWED_REFS = new Set(IMA_RUNTIME_REFS);
...
function registerCredentialApi(ctx) {
	return ctx.dshLoader.web.register(IMA_CREDENTIAL_API_PREFIX, async (request, response) => {
		...
		try {
			...
			const credentials = ctx.dshLoader.services.get("credentials");
			if (credentials === void 0) throw new ApiError(503, "service-unavailable", "credential service is unavailable");
			if (pathname === `/api/ima-copilot/credentials/describe`) {
				const entries = await Promise.all(IMA_RUNTIME_REFS.map(async (ref) => {
					...
					return [ref, await credentials.describe(branded)];
				}));
				writeOk(response, { credentials: Object.fromEntries(entries) });
				return;
			}
			if (pathname === `/api/ima-copilot/credentials/set`) {
				const values = readValues(await readJsonBody(request));
...
function readValues(payload) {
	if (payload === null || typeof payload !== "object" || Array.isArray(payload)) throw new ApiError(400, "bad-request", "request body must be an object");
	const candidate = payload.values;
	if (candidate === null || typeof candidate !== "object" || Array.isArray(candidate)) throw new ApiError(400, "bad-request", "values must be an object");
	const values = {};
	for (const [ref, value] of Object.entries(candidate)) {
		if (!ALLOWED_REFS.has(ref)) throw new ApiError(400, "bad-request", `unsupported credential reference ${ref}`);
		if (typeof value !== "string" || value.trim().length === 0) throw new ApiError(400, "bad-request", `credential ${ref} must be a non-empty string`);
		values[ref] = value;
	}
...
function normalizeValue(ref, value) {
	if (ref === "IMA_X_IMA_COOKIE") return normalizeImaHeaderCredential(value, "X-Ima-Cookie");
	if (ref === "IMA_X_IMA_BKN") return normalizeImaHeaderCredential(value, "X-Ima-Bkn");
	if (ref === "IMA_KNOWLEDGE_BASE_IDS") return value.trim();
...
function isTrustedRequest(ctx, request) {
	...
	const trusted = ctx.dshLoader.services.get("webRuntime")?.trustedHosts ?? [];
...
*/
function createImaTool(ctx, config, client) {
...
*/
async function resolveRuntimeState(ctx) {
	const credentials = ctx.dshLoader.services.get("credentials");
	if (credentials === void 0) throw new Error("ima_ask: credential service is unavailable");
...
function apply(ctx, config) {
	...
	const registerTool = () => {
		disposeTool();
		disposeTool = () => {};
		if (runtimeCtx === void 0) return;
		const tools = runtimeCtx.dshLoader.services.get("tools");
	...
	ctx.inject(["settings"], (settingsCtx) => {
		const scope = settingsCtx.dshLoader.settings.register(settingsCtx.dshLoader.settings.namespace(IMA_SETTINGS_NAMESPACE), Config, {
			base: config,
			validate: (value) => {
				resolveConfig(value);
			}
	...
	ctx.effect(() => registerCredentialApi(ctx), "ima-copilot: credential settings API");
}
//#endregion
